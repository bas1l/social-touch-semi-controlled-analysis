"""AnalysisRunnerGUI configuration — WorkflowEntry dataclass and YAML parser."""
...
@dataclass
class WorkflowEntry:
    """A single workflow declared in analysis_runner_gui.yaml.
...
def parse_runner_config(yaml_path: Path, project_root: Path) -> list[WorkflowEntry]:
    """Load analysis_runner_gui.yaml and return validated WorkflowEntry instances.
    ...
    Args:
        yaml_path: Path to the ``analysis_runner_gui.yaml`` file.
    ...
    for category_block in data.get("categories", []):
        ...
        for wf in category_block.get("workflows", []):
            ...
            if not script.exists():
                warnings.warn(
                    f"AnalysisRunnerGUI: skipping '{name}' — script not found: {script}",
                    stacklevel=2,
                )
                continue
            if dag_config is not None and not dag_config.exists():
                warnings.warn(
                    f"AnalysisRunnerGUI: skipping '{name}' — dag_config not found: {dag_config}",
                    stacklevel=2,
                )
                continue
