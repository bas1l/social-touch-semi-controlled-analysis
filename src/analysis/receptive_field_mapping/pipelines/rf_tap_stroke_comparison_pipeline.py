...
def run_tap_stroke_comparison(
    ...
    Each discovered ``<method>/`` produces its own tap-stroke comparison tree under
    ``output_dir/<method>/`` via identical per-method logic (:func:`_run_for_method`),
    so methods never overwrite each other. Boundary geometry comes from the
...
def _run_for_method(
    ...
    for csv_path, db_path_item in session_configs:
        ...
        if method_name not in session_records:
            logger.warning(
                "[RF Tap-Stroke Comparison] %s: no %s boundary folder — skipping session.",
