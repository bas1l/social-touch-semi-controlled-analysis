...
class PopulationData:
    ...
    cp_iff: np.ndarray                  # (C,) float32 — IFF at that frame
...
def _load_population_cache(
    ...
    return PopulationData(
        forearm_vertices=forearm_vertices.astype(np.float64, copy=False),
        touch_triple_keys=touch_triple_keys.astype(np.int64, copy=False),
        gesture_types=gesture_types.astype(object),
        spike_elicited=spike_elicited.astype(bool, copy=False),
        feature_names=feature_names,
        feature_matrix=feature_matrix.astype(np.float64, copy=False),
        cp_vertex_idx=cp_vertex_idx.astype(np.int64, copy=False),
        cp_touch_idx=cp_touch_idx.astype(np.int64, copy=False),
        cp_iff=cp_iff.astype(np.float32, copy=False),
        cp_spike=cp_spike.astype(bool, copy=False),
    )
...
def load_population_data(
    ...
    if cached is not None:
        needs_features = (
            touch_features_dir is not None
            and len(cached.feature_names) == 0
            and touch_features_dir.is_dir()
        )
        if needs_features:
            tag = series_csv_path.stem
            print(
                f"{_ts()} | [Population] [{tag}]: cache hit — supplementing "
                f"Stage 3 features...",
                flush=True,
            )
            touch_keys = cached.touch_triple_keys[:, 1:]
            ...
            print(
                ...
                f"{time.perf_counter() - t_feat:.1f}s  features={len(feature_names)}",
                flush=True,
            )
            if feature_names:
                cached.feature_names = feature_names
                cached.feature_matrix = feature_matrix
                _save_population_cache(series_csv_path, cached)
            print(
                f"{_ts()} | [Population] [{tag}]: cache hit "
                f"(+features: {time.perf_counter() - t_session:.2f}s)",
                flush=True,
            )
        else:
            print(
    ...
    for (block_order_id, trial_id, single_touch_id), grp in df.groupby(group_keys, sort=True):
        cp_strings = grp[CONTACT_POINTS_COL].fillna("[]").values
        spikes_arr = grp[NERVE_SPIKE_COL].to_numpy(dtype=bool)
        iff_arr = grp[NERVE_FREQ_COL].to_numpy(dtype=np.float64)
    ...
    for ti, rec in enumerate(touch_records):
        ...
        for ui in range(n_unique):
            ...
            for row_in_run in range(rl):
                ...
                touch_iff_parts.append(np.full(n_vtx, iff_val, dtype=np.float32))
    ...
    if all_cp_vertex_idx_parts:
        ...
        cp_iff = np.concatenate(all_cp_iff_parts).astype(np.float32, copy=False)
