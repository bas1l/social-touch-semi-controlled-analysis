...
from analysis.pipeline.shared_constants import NERVE_SPIKE_COL, CONTACT_POINTS_COL, NERVE_FREQ_COL
...
def load_explorer_data(
    ...
    for required_col in (CONTACT_POINTS_COL, NERVE_FREQ_COL):
        if required_col not in df.columns:
            raise ValueError(
                f"load_explorer_data: '{required_col}' column missing from {series_csv_path}"
            )

    cp_raw = df[CONTACT_POINTS_COL].fillna("[]").values
    ...
    spikes = df[NERVE_SPIKE_COL].to_numpy(dtype=bool)
