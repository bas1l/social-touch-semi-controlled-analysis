...
- [x] 1.1 — Add `"boundary_method"` entry to `_OPTION_ENUMS` (after `"contour_color"`, before closing `}`)
  ```python
  "boundary_method": [
  ...
  ```
- [x] 1.2 — Add `_OPTION_VISIBILITY` constant after `_OPTION_ENUMS`:
  ```python
  ...
  ```
- [x] 1.3 — Add `self._sections: dict[str, QWidget] = {}` to `__init__`, after `self._task_name`.
...
- [x] 2.1 — In `show_task()`, add `self._sections.clear()` immediately after the `deleteLater()` while-loop.
- [x] 2.2 — In `show_task()`, add `self._sections[key] = widget` on the line before every `self._layout.insertWidget(i, widget)` call.
- [x] 2.3 — In `show_task()`, add the initial visibility pass after the loop:
  ```python
  ...
  ```
- [x] 2.4 — In `_make_enum_handler()`, add the live-update block after `self.task_changed.emit()`:
  ```python
...
- docs/development/plans/active/dag-launcher-boundary-method-dropdown.md
