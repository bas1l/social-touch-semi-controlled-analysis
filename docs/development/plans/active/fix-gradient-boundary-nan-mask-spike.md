...
- [x] In `rf_gradient_boundary.py` line 99, change
  `filled = np.where(nan_mask, 0.0, smoothed)` →
  `filled = np.where(np.isnan(smoothed), 0.0, smoothed)`.
- [x] Verify `nan_mask` is still used on the output re-masking line (line 103); no change needed.
...
- [x] In `fig2_radial_profiling` (sandbox), after `ax_map.imshow(...)`, overlay the `grid_z`
...
- [x] Add `'data boundary'` to the map panel's legend entry.
...
- docs/development/plans/active/fix-gradient-boundary-nan-mask-spike.md
- scripts/sandbox_gradient_ridge_boundary.py
- src/analysis/receptive_field_mapping/metrics/rf_gradient_boundary.py
